...
class DeletedNotificationsList(NotificationViewList):
    def get_queryset(self):
        return self.request.user.notifications.deleted()
...
@never_cache
def live_deleted_notification_count(request):
    if not request.user.is_authenticated:
        data = {"unread_count": 0}
    else:
        data = {
            "unread_count": request.user.notifications.deleted().count(),
        }
    return JsonResponse(data)
...
@never_cache
def live_deleted_notification_list(request):
    """Return a json with a unread notification list"""
    if not request.user.is_authenticated:
        data = {"unread_count": 0, "unread_list": []}
        return JsonResponse(data)

    deleted_list = get_notification_list(request, "deleted")

    data = {
        "deleted_count": request.user.notifications.deleted().count(),
